...
export async function getArtDataCompVariant(artikul) {
  ...
  const yumiAction = compVariant?.competitorsLinks?.yumiLink
    ? getArtDataYumi(compVariant?.competitorsLinks?.yumiLink)
    : Promise.resolve(null);
  const sharteAction = compVariant?.competitorsLinks?.sharteLink
    ? getArtDataSharte(compVariant?.competitorsLinks?.sharteLink)
    : Promise.resolve(null);
  const airAction = compVariant?.competitorsLinks?.airLink
    ? getArtDataAir(compVariant?.competitorsLinks?.airLink)
    : Promise.resolve(null);
  const bestAction = compVariant?.competitorsLinks?.bestLink
    ? getArtDataBest(compVariant?.competitorsLinks?.bestLink)
    : Promise.resolve(null);

  const aeroAction = compVariant?.competitorsLinks?.aeroLink
    ? getArtDataAero(compVariant?.competitorsLinks?.aeroLink)
    : Promise.resolve(null);
  const balunAction = compVariant?.competitorsLinks?.balunLink
    ? getArtDataBalun(compVariant?.competitorsLinks?.balunLink)
    : Promise.resolve(null);
  const svyatoAction = compVariant?.competitorsLinks?.svyatoLink
    ? getArtDataSvyato(compVariant?.competitorsLinks?.svyatoLink)
    : Promise.resolve(null);
  ...
  let yumi, sharte, air, best, aero, balun, svyato, idea, chudo;
  ...
  if (chudoData && chudoData.status === "fulfilled" && chudoData.value) {
    chudo = {
      quant: chudoData.value?.quant,
      price: chudoData.value?.price,
    };
  } else {
    chudo = {
      quant: "N/A",
